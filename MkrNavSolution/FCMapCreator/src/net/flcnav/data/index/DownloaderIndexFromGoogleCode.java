...
import net.flcnav.data.index.ExtractGooglecodeAuthorization.GooglecodeUploadTokens;
...
public class DownloaderIndexFromGoogleCode {
	...
	private static Map<String, String> getContent(Map<String, String> files,
		...
		while (downloadNext) {
			...
			try {
				URL url = new URL(
						"http://code.google.com/p/flcnav/downloads/list?num=" + num + "&start=" + start); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public static URL getInputStreamToLoadIndex(String indexName) throws IOException{
		URL url = new URL("http://flcnav.googlecode.com/files/"+indexName); //$NON-NLS-1$
		return url;
	}
	...
	public static String deleteFileFromGoogleDownloads(String fileName, GooglecodeUploadTokens ggtokens) throws IOException {
		...
		String urlText = "http://code.google.com/p/flcnav/downloads/delete.do?name="+fileName; //$NON-NLS-1$
