...
import net.flcnav.binary.BinaryMapRouteReaderAdapter.RouteTypeRule;
...
public class RouteCalculationResult {
	...
	protected int nextAlarmInfo = 0;
		...
		public RouteCalculationResult(List<RouteSegmentResult> list, Location start, LatLon end, 
			RoutingContext ctx, boolean leftSide) {
	...
	private List<RouteSegmentResult> convertVectorResult(List<RouteDirectionInfo> directions, List<Location> locations, List<RouteSegmentResult> list,
			List<AlarmInfo> alarms, RoutingContext ctx) {
		...
		for (int routeInd = 0; routeInd < list.size(); routeInd++) {
			...
			if(turn != null) {
				...
				if(directions.size() > 0 && prevDirectionTime > 0 && prevDirectionDistance > 0) {
					...
					prev.setDescriptionRoute(prev.getDescriptionRoute() + " " + FCNavFormatter.getFormattedDistance(prevDirectionDistance, ctx));
	...
	protected void addMissingTurnsToRoute(List<Location> locations, 
			List<RouteDirectionInfo> originalDirections, Location start, LatLon end, /*ApplicationMode mode,*/ RoutingContext ctx,
			boolean leftSide){
		...
		previousInfo.setDescriptionRoute("route_head"/*ctx.getString( R.string.route_head)*/);
		...
		for (int i = 1; i < locations.size() - 1; i++) {
			...
			if(delta > 45 && delta < 315){
				...
				if(delta < 60){
					...
					description ="Turn slightly left and go";// ctx.getString( /*R.string.route_tsll*/);
				} else if(delta < 120){
					type = TurnType.valueOf(TurnType.TL, leftSide);
					description = "Turn left and go";//ctx.getString( R.string.route_tl);
				} else if(delta < 150){
					type = TurnType.valueOf(TurnType.TSHL, leftSide);
					description = "Turn sharply left and go";//ctx.getString( R.string.route_tshl);
				} else if(delta < 210){
					type = TurnType.valueOf(TurnType.TU, leftSide);
					description = "Make U-turn and go";//ctx.getString( R.string.route_tu);
				} else if(delta < 240){
					description = "Turn sharply right and go";//ctx.getString( R.string.route_tshr);
					type = TurnType.valueOf(TurnType.TSHR, leftSide);
				} else if(delta < 300){
					description = "Turn right and go";//ctx.getString( R.string.route_tr);
					type = TurnType.valueOf(TurnType.TR, leftSide);
				} else {
					description = "Turn sharply left and go";//ctx.getString( R.string.route_tslr);
				...
				previousInfo.setDescriptionRoute(previousInfo.getDescriptionRoute()
						+ " " + FCNavFormatter.getFormattedDistance(previousInfo.distance, ctx)); //$NON-NLS-1$
	...
	public String toString(TurnType type, net.flcnav.router.RoutingContext ctx) {
		/*
