...
import net.flcnav.binary.FlcnavOdb.FlcNavPoiNameIndex.FlcNavPoiNameIndexData;
...
public class BinaryMapPoiReaderAdapter {
	...
	protected void readPoiIndex(PoiRegion region) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavPoiIndex.NAME_FIELD_NUMBER :
				region.name = codedIS.readString();
				break;
			case FlcnavOdb.FlcNavPoiIndex.BOUNDARIES_FIELD_NUMBER: {
				...
				break; 
			case FlcnavOdb.FlcNavPoiIndex.CATEGORIESTABLE_FIELD_NUMBER : {
				int length = codedIS.readRawVarint32();
				int oldLimit = codedIS.pushLimit(length);
				readCategory(region);
				codedIS.popLimit(oldLimit);
			} break;
			case FlcnavOdb.FlcNavPoiIndex.BOXES_FIELD_NUMBER :
	...
	private void readCategory(PoiRegion region) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavCategoryTable.CATEGORY_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.FlcNavCategoryTable.SUBCATEGORIES_FIELD_NUMBER :
	...
	protected void searchPoiByName( PoiRegion region, SearchRequest<Amenity> req) throws IOException {
		...
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavPoiIndex.NAMEINDEX_FIELD_NUMBER :
				int length = readInt();
				int oldLimit = codedIS.pushLimit(length);
				...
				codedIS.popLimit(oldLimit);
				break;
			case FlcnavOdb.FlcNavPoiIndex.POIDATA_FIELD_NUMBER :
	...
	private TIntLongHashMap readPoiNameIndex(Collator instance, String query, SearchRequest<Amenity> req) throws IOException {
		...
		while(true){
			...
			case 0:
				return offsets;
			case FlcnavOdb.FlcNavPoiNameIndex.TABLE_FIELD_NUMBER : {
				int length = readInt();
				int oldLimit = codedIS.pushLimit(length);
				...
				codedIS.popLimit(oldLimit);
				break; }
			case FlcnavOdb.FlcNavPoiNameIndex.DATA_FIELD_NUMBER : {
				if(dataOffsets != null){
	...
	private void readPoiNameIndexData(TIntLongHashMap offsets, SearchRequest<Amenity> req) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcNavPoiNameIndexData.ATOMS_FIELD_NUMBER :
				int len = codedIS.readRawVarint32();
				int oldLim = codedIS.pushLimit(len);
	...
	private void readPoiNameIndexDataAtom(TIntLongHashMap offsets, SearchRequest<Amenity> req) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavPoiNameIndexDataAtom.X_FIELD_NUMBER :
				x = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiNameIndexDataAtom.Y_FIELD_NUMBER :
				y = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiNameIndexDataAtom.ZOOM_FIELD_NUMBER :
				zoom = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiNameIndexDataAtom.SHIFTTO_FIELD_NUMBER :
	...
	private void readPoiData(CollatorStringMatcher matcher, SearchRequest<Amenity> req, PoiRegion region) throws IOException {
		...
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavPoiBoxData.X_FIELD_NUMBER :
				x = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiBoxData.ZOOM_FIELD_NUMBER :
				zoom = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiBoxData.Y_FIELD_NUMBER :
				y = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavPoiBoxData.POIDATA_FIELD_NUMBER:
				int len = codedIS.readRawVarint32();
				int oldLim = codedIS.pushLimit(len);
	...
	private Amenity readPoiPoint(int left31, int right31, int top31, int bottom31, 
		...
		while(true){
			int t = codedIS.readTag();
			int tag = WireFormat.getTagFieldNumber(t);
			if(amenityType == null && (tag > FlcnavOdb.FlcNavPoiBoxDataAtom.CATEGORIES_FIELD_NUMBER || tag == 0)) {
				codedIS.skipRawBytes(codedIS.getBytesUntilLimit());
				return null;
			}
			switch (tag) {
			case 0:
				...
				return am;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.DX_FIELD_NUMBER :
				x = (codedIS.readSInt32() + (px << (24 - zoom))) << 7;
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.DY_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.CATEGORIES_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.ID_FIELD_NUMBER :
				am.setId(codedIS.readUInt64());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.NAME_FIELD_NUMBER :
				am.setName(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.NAMEEN_FIELD_NUMBER :
				am.setEnName(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.OPENINGHOURS_FIELD_NUMBER :
				am.setOpeningHours(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.SITE_FIELD_NUMBER :
				am.setSite(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.PHONE_FIELD_NUMBER:
				am.setPhone(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavPoiBoxDataAtom.NOTE_FIELD_NUMBER:
	...
	private boolean checkCategories(SearchRequest<Amenity> req, PoiRegion region) throws IOException {
		while(true){
			...
			case 0:
				return false;
			case FlcnavOdb.FlcNavPoiCategories.CATEGORIES_FIELD_NUMBER:
				AmenityType type = AmenityType.OTHER;
				String subcat = "";
				int cat = codedIS.readUInt32();
				int subcatId = cat >> SHIFT_BITS_CATEGORY;
	...
	private boolean readBoxField(int left31, int right31, int top31, int bottom31,
		...
		while(true){
			...
			case FlcnavOdb.FlcNavPoiBox.ZOOM_FIELD_NUMBER :
				zoom = codedIS.readUInt32() + pzoom;
				break;
			case FlcnavOdb.FlcNavPoiBox.LEFT_FIELD_NUMBER :
				dx = codedIS.readSInt32();
				break;
			case FlcnavOdb.FlcNavPoiBox.TOP_FIELD_NUMBER:
				dy = codedIS.readSInt32();
				break;
			case FlcnavOdb.FlcNavPoiBox.CATEGORIES_FIELD_NUMBER:
			...
			case FlcnavOdb.FlcNavPoiBox.SUBBOXES_FIELD_NUMBER: {
			...
			case FlcnavOdb.FlcNavPoiBox.SHIFTTODATA_FIELD_NUMBER: {
				int x = dx + (px << (zoom - pzoom));
				int y = dy + (py << (zoom - pzoom));
