...
import net.flcnav.CollatorStringMatcher;
import net.flcnav.LogUtil;
import net.flcnav.CollatorStringMatcher.StringMatcherMode;
import net.flcnav.ResultMatcher;
import net.flcnav.StringMatcher;
import net.flcnav.binary.BinaryMapAddressReaderAdapter.AddressRegion;
import net.flcnav.binary.BinaryMapAddressReaderAdapter.CitiesBlock;
import net.flcnav.binary.BinaryMapPoiReaderAdapter.PoiRegion;
import net.flcnav.binary.BinaryMapRouteReaderAdapter.RouteRegion;
import net.flcnav.binary.BinaryMapRouteReaderAdapter.RouteSubregion;
import net.flcnav.binary.BinaryMapTransportReaderAdapter.TransportIndex;
...
import net.flcnav.data.TransportRoute;
...
// 	    this.collator.setStrength(Collator.PRIMARY); //ignores also case
	...
	private void init() throws IOException {
		...
		while(true){
			...
			case 0:
				...
				return;
			case FlcnavOdb.FlcNavStructure.VERSION_FIELD_NUMBER :
				version = codedIS.readUInt32();
				break;
			case FlcnavOdb.FlcNavStructure.DATECREATED_FIELD_NUMBER :
				dateCreated = codedIS.readInt64();
				log.info("Map was created " +new Date(dateCreated));
				break;
			case FlcnavOdb.FlcNavStructure.MAPINDEX_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.FlcNavStructure.ADDRESSINDEX_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.FlcNavStructure.TRANSPORTINDEX_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.FlcNavStructure.ROUTINGINDEX_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.FlcNavStructure.POIINDEX_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.FlcNavStructure.VERSIONCONFIRM_FIELD_NUMBER :
	 ...
	 * Transport public methods
	 */
	public List<net.flcnav.data.TransportRoute> getTransportRouteDescriptions(TransportStop stop) throws IOException {
		TransportIndex ind = getTransportIndex(stop.getFileOffset());
		if(ind == null){
			return null;
		}
		List<net.flcnav.data.TransportRoute> list = new ArrayList<TransportRoute>();
		TIntObjectHashMap<String> stringTable = new TIntObjectHashMap<String>();
	...
	private void readMapIndex(MapIndex index) throws IOException {
		...
		while(true){
			...
			case 0:
				...
				return;
			case FlcnavOdb.FlcNavMapIndex.NAME_FIELD_NUMBER :
				index.setName(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavMapIndex.RULES_FIELD_NUMBER :
				int len = codedIS.readInt32();
				int oldLimit = codedIS.pushLimit(len);
				readMapEncodingRule(index, defaultId++);
				codedIS.popLimit(oldLimit);
				break;
			case FlcnavOdb.FlcNavMapIndex.LEVELS_FIELD_NUMBER :
				int length = readInt();
	...
	private BinaryMapDataObject readMapDataObject(MapTree tree , SearchRequest<BinaryMapDataObject> req, 
		...
		boolean area = FlcnavOdb.MapData.AREACOORDINATES_FIELD_NUMBER == tag;
		if(!area && FlcnavOdb.MapData.COORDINATES_FIELD_NUMBER != tag) {
		...
		while (loop) {
			...
			case 0:
				loop = false;
				break;
			case FlcnavOdb.MapData.POLYGONINNERCOORDINATES_FIELD_NUMBER:
				...
				codedIS.popLimit(old);
				break;
			case FlcnavOdb.MapData.ADDITIONALTYPES_FIELD_NUMBER:
				...
				codedIS.popLimit(old);
				break;
			case FlcnavOdb.MapData.TYPES_FIELD_NUMBER:
				...
				break;
			case FlcnavOdb.MapData.ID_FIELD_NUMBER:
				id = codedIS.readSInt64();
				break;
			case FlcnavOdb.MapData.STRINGNAMES_FIELD_NUMBER:
	...
	protected List<String> readStringTable() throws IOException{
		...
		while(true){
			...
			case 0:
				return list;
			case FlcnavOdb.StringTable.S_FIELD_NUMBER :
	...
	public static void main(String[] args) throws IOException {
		//RandomAccessFile raf = new RandomAccessFile(new File("/home/victor/projects/FlcNav/data/osm-gen/Luxembourg.obf"), "r");
		RandomAccessFile raf = new RandomAccessFile(new File("C:\\my\\OSM_Maps\\obf\\ontario.obf"), "r");
...
//					return type == AmenityType.TRANSPORTATION && "fuel".equals(subcategory);
	...
	private static void testTransportSearch(BinaryMapIndexReader reader) throws IOException {
		...
		}
		{
			for (TransportStop s : reader.searchTransportIndex(buildSearchTransportRequest(sleft, sright, stop, sbottom, 15, null))) {
				...
				for (net.flcnav.data.TransportRoute route : routes.valueCollection()) {
	...
	int readIndexedStringTable(Collator instance, String query, String prefix, TIntArrayList list, int charMatches) throws IOException {
		...
		while(true){
			...
			case 0:
				return charMatches;
			case FlcnavOdb.IndexedStringTable.KEY_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.IndexedStringTable.VAL_FIELD_NUMBER :
				int val = readInt();
				if (key != null) {
					list.add(val);
				}
				break;
			case FlcnavOdb.IndexedStringTable.SUBTABLES_FIELD_NUMBER :
	...
	static String city_req_name="Toronto";
	static String street_req_name="Yonge";
	public static void testAddressSearchByName(BinaryMapIndexReader reader) throws IOException {
		...
		*/
		/*
		SearchRequest<MapObject> req = buildAddressByNameRequest(null, "Yonge");
		List<MapObject> list=reader.searchAddressDataByName(req);
		if(list!=null)
		{
		    for(MapObject mo:list)
		    {
		    	String name=mo.getEnName();
		    	LatLon loc=mo.getLocation();
		...
		*/
		...
		//SearchRequest<MapObject> req = buildAddressByNameRequest(null, "Toronto");
		SearchRequest<City> request = new SearchRequest<City>();
		
		request.resultMatcher = new ResultMatcher<City>() {
			 
			@Override
			public boolean publish(City c) {
				String name=c.getEnName();
				if(name.contains(city_req_name))//if(collator.equals(name,city_req_name))
					return true;
				else
				return false;
			}
		...
		request.nameQuery = "Toronto";
		
		
		List<City> cs = reader.getCities(reg, request, BinaryMapAddressReaderAdapter.CITY_TOWN_TYPE);
		for(City c : cs){
			int buildings = 0;
			
			SearchRequest<Street> street_req = new SearchRequest<Street>();
			
			street_req.resultMatcher = new ResultMatcher<Street>() {
			...
			reader.preloadStreets(c, street_req);
	...
	private static void _testAddressSearch(BinaryMapIndexReader reader) throws IOException {
