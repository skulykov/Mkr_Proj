...
import static net.flcnav.binary.BinaryMapIndexReader.TRANSPORT_STOP_ZOOM;
...
public class BinaryMapTransportReaderAdapter {
	...
	protected void readTransportIndex(TransportIndex ind) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.FlcNavTransportIndex.ROUTES_FIELD_NUMBER :
				skipUnknownField(t);
				break;
			case FlcnavOdb.FlcNavTransportIndex.NAME_FIELD_NUMBER :
				ind.setName(codedIS.readString());
				break;
			case FlcnavOdb.FlcNavTransportIndex.STOPS_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.FlcNavTransportIndex.STRINGTABLE_FIELD_NUMBER :
	...
	private void readTransportBounds(TransportIndex ind) throws IOException {
		while(true){
			...
			case 0:
				return;
			case FlcnavOdb.TransportStopsTree.LEFT_FIELD_NUMBER :
				ind.left = codedIS.readSInt32();
				break;
			case FlcnavOdb.TransportStopsTree.RIGHT_FIELD_NUMBER :
				ind.right = codedIS.readSInt32(); 
				break;
			case FlcnavOdb.TransportStopsTree.TOP_FIELD_NUMBER :
				ind.top = codedIS.readSInt32();
				break;
			case FlcnavOdb.TransportStopsTree.BOTTOM_FIELD_NUMBER :
	...
	protected void searchTransportTreeBounds(int pleft, int pright, int ptop, int pbottom,
		...
		while(true){
			...
			case FlcnavOdb.TransportStopsTree.TOP_FIELD_NUMBER :
				...
				init |= 8;
				break;
			case FlcnavOdb.TransportStopsTree.LEAFS_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.TransportStopsTree.SUBTREES_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.TransportStopsTree.BASEID_FIELD_NUMBER :
	...
	public net.flcnav.data.TransportRoute getTransportRoute(int filePointer, TIntObjectHashMap<String> stringTable,
		...
		net.flcnav.data.TransportRoute dataObject = new net.flcnav.data.TransportRoute();
		boolean end = false;
		long rid = 0;
		int rx = 0;
		int ry = 0;
		...
		while(!end){
			...
			case 0:
				...
				break;
			case FlcnavOdb.TransportRoute.DISTANCE_FIELD_NUMBER :
				dataObject.setDistance(codedIS.readUInt32());
				break;
			case FlcnavOdb.TransportRoute.ID_FIELD_NUMBER :
				dataObject.setId(codedIS.readUInt64());
				break;
			case FlcnavOdb.TransportRoute.REF_FIELD_NUMBER :
				dataObject.setRef(codedIS.readString());
				break;
			case FlcnavOdb.TransportRoute.TYPE_FIELD_NUMBER :
				dataObject.setType(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRoute.NAME_EN_FIELD_NUMBER :
				dataObject.setEnName(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRoute.NAME_FIELD_NUMBER :
				dataObject.setName(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRoute.OPERATOR_FIELD_NUMBER:
				dataObject.setOperator(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRoute.REVERSESTOPS_FIELD_NUMBER:
				if(onlyDescription){
					end = true;
				...
				break;
			case FlcnavOdb.TransportRoute.DIRECTSTOPS_FIELD_NUMBER:
				if(onlyDescription){
					end = true;
					codedIS.skipRawBytes(codedIS.getBytesUntilLimit());
					break;
	...
	protected void initializeNames(boolean onlyDescription, net.flcnav.data.TransportRoute dataObject,
	...
	private TransportStop readTransportRouteStop(int dx, int dy, long did, TIntObjectHashMap<String> stringTable) throws IOException {
		...
		while(!end){
			...
			case 0:
				...
				break;
			case FlcnavOdb.TransportRouteStop.NAME_EN_FIELD_NUMBER :
				dataObject.setEnName(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRouteStop.NAME_FIELD_NUMBER :
				dataObject.setName(regStr(stringTable)); //$NON-NLS-1$
				break;
			case FlcnavOdb.TransportRouteStop.ID_FIELD_NUMBER :
				did += codedIS.readSInt64();
				break;
			case FlcnavOdb.TransportRouteStop.DX_FIELD_NUMBER :
				dx += codedIS.readSInt32();
				break;
			case FlcnavOdb.TransportRouteStop.DY_FIELD_NUMBER :
	...
	private TransportStop readTransportStop(int shift, int cleft, int cright, int ctop, int cbottom, SearchRequest<TransportStop> req) throws IOException {
		...
		if(FlcnavOdb.TransportStop.DX_FIELD_NUMBER != tag) {
			throw new IllegalArgumentException();
		}
		int x = codedIS.readSInt32() + cleft;
		
		tag = WireFormat.getTagFieldNumber(codedIS.readTag());
		if(FlcnavOdb.TransportStop.DY_FIELD_NUMBER != tag) {
			throw new IllegalArgumentException();
		}
		...
		while(true){
			...
			case 0:
				...
				return dataObject;
			case FlcnavOdb.TransportStop.ROUTES_FIELD_NUMBER :
				req.cacheTypes.add(shift - codedIS.readUInt32());
				break;
			case FlcnavOdb.TransportStop.NAME_EN_FIELD_NUMBER :
				...
				break;
			case FlcnavOdb.TransportStop.NAME_FIELD_NUMBER :
				if (req.stringTable != null) {
				...
				break;
			case FlcnavOdb.TransportStop.ID_FIELD_NUMBER :
