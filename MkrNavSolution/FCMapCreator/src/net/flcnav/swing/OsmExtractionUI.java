...
import net.flcnav.MapCreatorVersion;
import net.flcnav.data.preparation.IndexCreator;
...
import net.flcnav.osm.io.OsmBoundsFilter;
import net.flcnav.osm.io.OsmStorageWriter;
import net.flcnav.swing.MapPanel.MapSelectionArea;
...
public class OsmExtractionUI implements IMapLocationListener {
	...
	public static final String LOG_PATH  = getUserLogDirectoryPath() + "/flcnav.log"; //$NON-NLS-1$ //$NON-NLS-2$
...
//	private TreeModelListener treeModelListener;
	...
	public void createUI(){
		frame = new JFrame(Messages.getString("OsmExtractionUI.FLCNAV_MAP_CREATOR")); //$NON-NLS-1$
	    try {
...
//		panel.add(normalizingStreets);
	...
	public void loadCountry(final File f, final IOsmStorageFilter filter){
		try {
			...
			dlg.run();
			frame.setTitle(Messages.getString("OsmExtractionUI.FLCNAV_MAP_CREATOR_FILE") + f.getName()); //$NON-NLS-1$
		} catch (InterruptedException e1) {
			log.error("Interrupted", e1);  //$NON-NLS-1$
		} catch (InvocationTargetException e1) {
