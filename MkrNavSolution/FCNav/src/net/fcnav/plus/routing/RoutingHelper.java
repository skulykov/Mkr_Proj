...
public class RoutingHelper {
	...
	public RoutingHelper(FCNavSettings settings, Context context, CommandPlayer player){
	...
	public String getGeneralRouteInformation(){
		...
		return context.getString(R.string.route_general_information, FCNavFormatter.getFormattedDistance(dist, context),
				hours, minutes);
	}
	...
	public String formatStreetName(String name, String ref) {
		if(name != null && name.length() > 0){
			...
			if(ref != null && ref.length() > 0) {
				name += "(" + ref +")";    // svk what's this ref for???
			}*/
			return name;
		} else {
			return ref;
	...
	private class RouteRecalculationThread extends Thread implements Interruptable {
		...
		@Override
		public void run() {
			boolean leftSide =false; //settings.LEFT_SIDE_NAVIGATION.get();
			...
			if (res.isCalculated()) {
				showMessage(context.getString(R.string.new_route_calculated_dist)
						+ ": " + FCNavFormatter.getFormattedDistance(res.getWholeDistance(), context)); //$NON-NLS-1$
			} else if (service != RouteService.FLCNAV && !settings.isInternetConnectionAvailable()) {
	...
	public GPXFile generateGPXFileWithRoute(){
		return provider.createFlcnavRouterGPX(route);
	}
	//-----------------------------------------------------
