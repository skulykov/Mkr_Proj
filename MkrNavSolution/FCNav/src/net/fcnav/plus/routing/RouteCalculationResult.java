	...
	private List<RouteSegmentResult> convertVectorResult(List<RouteDirectionInfo> directions, List<Location> locations, List<RouteSegmentResult> list,
		...
		for (int routeInd = 0; routeInd < list.size(); routeInd++) {
			...
			if(turn != null) {
				...
				String streetName=null;  //svk
				if(routeInd + 1< list.size()) {
					//RouteSegmentResult next = list.get(routeInd+1); // svk list.get(routeInd);
					//info.setRef(next.getObject().getRef());
					//streetName=next.getObject().getName();
					//info.setStreetName(streetName);
					
					info.setRef(s.getObject().getRef());
					streetName=s.getObject().getName();
					if(streetName== null && routeInd==0)
					{
						RouteSegmentResult next = list.get(routeInd+1);
						streetName=next.getObject().getName();
					}
					info.setStreetName(streetName);
				...
				{
					String str=info.getDescriptionRoute();
					str=str+" - "+streetName;
					info.setDescriptionRoute(str);
				}
