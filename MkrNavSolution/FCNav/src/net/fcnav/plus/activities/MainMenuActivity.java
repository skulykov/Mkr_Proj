...
import net.flcnav.plus.render.MapRenderRepositories;
...
public class MainMenuActivity extends Activity {
	...
	public void checkPreviousRunsForExceptions(boolean firstTime) {
		...
		final File file = ((FCNavApplication) getApplication()).getSettings().extendFlcnavPath(FCNavApplication.EXCEPTION_PATH);
		if (file.exists() && file.length() > 0) {
			if (size != file.length() && !firstTime) {
				String msg = MessageFormat.format(getString(R.string.previous_run_crashed), FCNavApplication.EXCEPTION_PATH);
				...
				builder.setPositiveButton(R.string.send_report, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						intent.putExtra(Intent.EXTRA_EMAIL, new String[] { "fcnav@gmail.com" }); //$NON-NLS-1$
						intent.putExtra(Intent.EXTRA_STREAM, Uri.fromFile(file));
						intent.setType("vnd.android.cursor.dir/email"); //$NON-NLS-1$
						intent.putExtra(Intent.EXTRA_SUBJECT, "FCNav bug"); //$NON-NLS-1$
	...
	public static void onCreateMainMenu(Window window, final Activity activity){
		// I get rid of main menu and move it to toolbar
		// map must be active right away
		/*
		...
		SharedPreferences prefs = activity.getApplicationContext().getSharedPreferences("net.flcnav.settings", MODE_WORLD_READABLE);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(getIntent() != null){
			...
			if(b!=null && b.containsKey("key"))
			{
				...
				if(value==1)
				{
					...
					//log.debug("Action tracker Got Request: "+str);
					if(str!=null && str!="")
					{
						flag=true;
					}
		...
		showMap.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				final Intent mapIndent = new Intent(activity, FCNavIntents.getMapActivity());
		...
		settingsButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				final Intent settings = new Intent(activity, FCNavIntents.getSettingsActivity());
		...
		favouritesButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				final Intent favorites = new Intent(activity, FCNavIntents.getFavoritesActivity());
		...
		searchButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				final Intent search = new Intent(activity, FCNavIntents.getSearchActivity());
		...
		*/
		...
		//svk -- just activate Map
		final Activity activity = this;
		final Intent mapIndent = new Intent(activity, FCNavIntents.getMapActivity());
		if(flag)
			mapIndent.putExtras(b);
		activity.startActivityForResult(mapIndent, 0);
	}
/*
	private void applicationInstalledFirstTime() {
		boolean netFlcnavWasInstalled = false;
		try {
			ApplicationInfo applicationInfo = getPackageManager().getApplicationInfo("net.flcnav", PackageManager.GET_META_DATA);
			netFlcnavWasInstalled = applicationInfo != null;
		} catch (NameNotFoundException e) {
			netFlcnavWasInstalled = false;
		}
		
		if(netFlcnavWasInstalled){
			Builder builder = new AccessibleAlertBuilder(this);
			builder.setMessage(R.string.flcnav_net_previously_installed);
