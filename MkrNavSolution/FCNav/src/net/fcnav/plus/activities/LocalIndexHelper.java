...
import net.fcnav.GPXUtilities.Track;
import net.fcnav.GPXUtilities.TrkSegment;
import net.fcnav.GPXUtilities.WptPt;
import net.fcnav.FCNavFormatter;
import net.flcnav.binary.BinaryIndexPart;
import net.flcnav.binary.BinaryMapAddressReaderAdapter.AddressRegion;
import net.flcnav.binary.BinaryMapIndexReader;
import net.flcnav.binary.BinaryMapIndexReader.MapIndex;
import net.flcnav.binary.BinaryMapIndexReader.MapRoot;
...
import net.fcnav.plus.activities.LocalIndexesActivity.LoadLocalIndexTask;
import net.fcnav.plus.voice.MediaCommandPlayerImpl;
import net.fcnav.plus.voice.TTSCommandPlayerImpl;
...
public class LocalIndexHelper {
	...
	public LocalIndexHelper(FCNavApplication app){
		this.app = app;
	}
...
//				}
	...
	private void updateGpxInfo(LocalIndexInfo info, File f) {
		...
		if(result.warning != null){
			...
			info.setDescription(app.getString(R.string.local_index_gpx_info, totalTracks, points,
					result.points.size(), FCNavFormatter.getFormattedDistance(totalDistance, app),
					startTime, endTime));
			...
			if(speedCount > 0){
				info.setDescription(info.getDescription() +  
						...
						FCNavFormatter.getFormattedSpeed((float) (totalSpeedSum / speedCount), app),
						FCNavFormatter.getFormattedSpeed(maxSpeed, app)));
	...
	public List<LocalIndexInfo> getLocalIndexData(LocalIndexType type, LoadLocalIndexTask loadTask){
		...
		loadObfData(settings.extendFlcnavPath(ResourceManager.MAPS_PATH), result, false, loadTask, loadedMaps);
		loadObfData(settings.extendFlcnavPath(ResourceManager.BACKUP_PATH), result, true, loadTask, loadedMaps);
		loadTilesData(settings.extendFlcnavPath(ResourceManager.TILES_PATH), result, false, loadTask);
		loadTilesData(settings.extendFlcnavPath(ResourceManager.BACKUP_PATH), result, false, loadTask);
		loadPoiData(settings.extendFlcnavPath(ResourceManager.POI_PATH), result, false, loadTask);
		loadPoiData(settings.extendFlcnavPath(ResourceManager.BACKUP_PATH), result, true, loadTask);
		loadVoiceData(settings.extendFlcnavPath(ResourceManager.VOICE_PATH), result, false, loadTask);
		loadVoiceData(settings.extendFlcnavPath(ResourceManager.BACKUP_PATH), result, true, loadTask);
		loadGPXData(settings.extendFlcnavPath(ResourceManager.GPX_PATH), result, false, loadTask);
		loadGPXData(settings.extendFlcnavPath(ResourceManager.BACKUP_PATH), result, true, loadTask);
