...
public class ContributionVersionActivity extends FCNavListActivity {
	...
	private static final String URL_TO_RETRIEVE_BUILDS = "http://download.fcnav.net/builds.php";
	private static final String URL_GET_BUILD = "http://download.fcnav.net/";
	
	private ProgressDialog progressDlg;
	private Date currentInstalledDate;

	private List<FlcNavBuild> downloadedBuilds = new ArrayList<FlcNavBuild>();
	private SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy");
	private File pathToDownload = new File(Environment.getExternalStorageDirectory(), ResourceManager.APP_DIR + "flcnavToInstall.apk");
	private FlcNavBuild currentSelectedBuild = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String installDate = ((FCNavApplication) getApplicationContext()).getSettings().CONTRIBUTION_INSTALL_APP_DATE.get();
		if(installDate != null){
			try {
...
//		progressFileDlg.setCancelable(false);
	...
	protected void endThreadOperation(int operationId, Exception e){
		...
		if(operationId == DOWNLOAD_BUILDS_LIST){
			if(e != null){
				...
				setListAdapter(new FlcnavBuildsAdapter(downloadedBuilds));
			}
	...
	private void updateInstalledApp(boolean showMessage, Date d) {
		...
		((FCNavApplication) getApplicationContext()).getSettings().CONTRIBUTION_INSTALL_APP_DATE.set(dateFormat.format(d));
	}
		...
		if(operationId == DOWNLOAD_BUILDS_LIST){
			...
			while((next = parser.next()) != XmlPullParser.END_DOCUMENT) {
				if(next == XmlPullParser.START_TAG && parser.getName().equals("build")) { //$NON-NLS-1$
					if ("flcnav".equalsIgnoreCase(parser.getAttributeValue(null, "type"))) {
						...
						FlcNavBuild build = new FlcNavBuild(path, size, d, tag);
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		final FlcNavBuild item = (FlcNavBuild) getListAdapter().getItem(position);
	...
	@Override
	public FlcnavBuildsAdapter getListAdapter() {
		return (FlcnavBuildsAdapter) super.getListAdapter();
	}
	...
	protected class FlcnavBuildsAdapter extends ArrayAdapter<FlcNavBuild> implements Filterable {
		

		public FlcnavBuildsAdapter(List<FlcNavBuild> builds) {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (v == null) {
				...
				v = inflater.inflate(net.fcnav.plus.R.layout.download_build_list_item, parent, false);
			}
			final View row = v;
			FlcNavBuild build = getItem(position);
	...
	private static class FlcNavBuild {
		public String path;
		public String size;
		public Date date;
		public String tag;
		
		public FlcNavBuild(String path, String size, Date date, String tag) {
			super();
			this.path = path;
			this.size = size;
			this.date = date;
