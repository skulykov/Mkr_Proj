...
import net.fcnav.plus.TransportIndexRepository;
import net.fcnav.plus.TransportIndexRepository.RouteInfoLocation;
import net.fcnav.plus.activities.TransportRouteHelper;
...
public class SearchTransportActivity extends ListActivity implements SearchActivityChild {
	...
	public void searchTransport(){
		...
		if (!routeCalculated && getLocationToStart() != null) {
			...
			List<TransportIndexRepository> rs = ((FCNavApplication)getApplication()).getResourceManager().searchTransportRepositories(locationToStart.getLatitude(), 
	...
	public String getInformation(RouteInfoLocation route, List<TransportStop> stops, int position, boolean part){
		...
		text.append(getString(R.string.transport_route_distance)).append(" ").append(FCNavFormatter.getFormattedDistance((int) dist, this));  //$NON-NLS-1$/
		if(!part){
			...
			if (endStop != null) {
				String before = FCNavFormatter.getFormattedDistance((int) MapUtils.getDistance(endStop, route.getStart().getLocation()), SearchTransportActivity.this);
			...
			if (stStop != null) {
				String after = FCNavFormatter.getFormattedDistance((int) MapUtils.getDistance(stStop, route.getStop().getLocation()), SearchTransportActivity.this);
	...
	public void onListItemClick(ListView parent, View v, int position, long id) {
		...
		for(TransportStop st : stops){
			...
			if(locationToGo != null){
				...
				n.append(FCNavFormatter.getFormattedDistance((int) MapUtils.getDistance(locationToGo, st.getLocation()), SearchTransportActivity.this)).append("]"); //$NON-NLS-1$ 
			} else if(locationToStart != null){
				n.append("[").append(FCNavFormatter.getFormattedDistance((int) MapUtils.getDistance(locationToStart, st.getLocation()), SearchTransportActivity.this)).append("] - "); //$NON-NLS-1$ //$NON-NLS-2$
				n.append(name);
			} else {
				n.append(name);
			}
	...
	class TransportStopAdapter extends ArrayAdapter<RouteInfoLocation> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (locationToGo != null) {
				labelW.append(FCNavFormatter.getFormattedDistance(stop.getDistToLocation(), SearchTransportActivity.this));
			} else {
			...
			distanceLabel.setText(" " + FCNavFormatter.getFormattedDistance(dist, SearchTransportActivity.this)); //$NON-NLS-1$
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if(info == null){
				...
				if(st != null && end != null){
					...
					text.setText(MessageFormat.format(getString(R.string.transport_searching_route), FCNavFormatter.getFormattedDistance(dist, SearchTransportActivity.this)));
				} else {
			...
			if (currentRouteLocation == -1) {
				...
				labelW.append(getString(R.string.transport_to_go_before)).append(" ").append(FCNavFormatter.getFormattedDistance(startDist, SearchTransportActivity.this)); //$NON-NLS-1$
				if (position == getCount() - 1) {
					LatLon stop = getStartStop(position + 1);
					if(stop != null) {
						int endDist = (int) MapUtils.getDistance(stop, info.getStop().getLocation());
						labelW.append(", ").append(getString(R.string.transport_to_go_after)).append(" ").append(FCNavFormatter.getFormattedDistance(endDist, SearchTransportActivity.this));  //$NON-NLS-1$ //$NON-NLS-2$
					}
				}
