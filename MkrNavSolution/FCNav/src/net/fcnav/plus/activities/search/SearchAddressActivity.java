...
public class SearchAddressActivity extends Activity {
	...
	private FCNavSettings fcnavSettings;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		fcnavSettings = ((FCNavApplication) getApplication()).getSettings();
		attachListeners();
	}
	...
	public void showOnMap(boolean navigateTo) {
		...
		if(selectAddressMode){
			...
			if (navigateTo) {
				fcnavSettings.setPointToNavigate(searchPoint.getLatitude(), searchPoint.getLongitude(), historyName);
			} else {
				fcnavSettings.setMapLocationToShow(searchPoint.getLatitude(), searchPoint.getLongitude(), zoom, historyName);
			}
	...
	public void loadData() {
		if (!Algoritms.isEmpty(region)) {
			String postcodeStr = fcnavSettings.getLastSearchedPostcode();
			if (!Algoritms.isEmpty(postcodeStr)) {
				postcode = postcodeStr;
			} else {
				city = fcnavSettings.getLastSearchedCityName();
			}

			if (!Algoritms.isEmpty(postcode) || !Algoritms.isEmpty(city)) {
				street = fcnavSettings.getLastSearchedStreet();
				if (!Algoritms.isEmpty(street)) {
					String str = fcnavSettings.getLastSearchedIntersectedStreet();
					radioBuilding = Algoritms.isEmpty(str);
					if (!radioBuilding) {
						street2 = str;
					} else {
						building = fcnavSettings.getLastSearchedBuilding();
					}
				}
			}
		}
	...
	protected void onResume() {
		...
		searchPoint = fcnavSettings.getLastSearchedPoint();
		...
		street = null;
		building = null;
		region = fcnavSettings.getLastSearchedRegion();
		RegionAddressRepository reg = ((FCNavApplication)getApplication()).getResourceManager().getRegionRepository(region);
		if(reg != null && reg.useEnglishNames() != fcnavSettings.USE_ENGLISH_NAMES.get()){
			reg.setUseEnglishNames(fcnavSettings.USE_ENGLISH_NAMES.get());
		}
		loadData();
		updateUI();
