...
import net.fcnav.plus.R.drawable;
...
public class NavigationService extends Service implements LocationListener {
	...
	public final static String FLCNAV_STOP_SERVICE_ACTION  = "FLCNAV_STOP_SERVICE_ACTION"; //$NON-NLS-1$
	...
	protected synchronized static PowerManager.WakeLock getLock(Context context) {
		if (lockStatic == null) {
			...
			lockStatic = mgr.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "FlcnavServiceLock");
		}
		return lockStatic;
	}
...
//		setForeground(true);   //===============================================svk 3.2 ???????? 
		...
		savingTrackHelper = ((FCNavApplication) getApplication()).getSavingTrackHelper();
		liveMonitoringHelper = ((FCNavApplication) getApplication()).getLiveMonitoringHelper();
		
		routingHelper = ((FCNavApplication)getApplication()).getRoutingHelper();
		((FCNavApplication)getApplication()).setNavigationService(this);
		...
		registerReceiver(broadcastReceiver, new IntentFilter(FLCNAV_STOP_SERVICE_ACTION));
		Intent notificationIntent = new Intent(FLCNAV_STOP_SERVICE_ACTION);
	...
	public void onDestroy() {
		super.onDestroy();
		((FCNavApplication)getApplication()).setNavigationService(null);
