...
import net.fcnav.access.MapExplorer;
...
import net.fcnav.plus.views.MultiTouchSupport.MultiTouchZoomListener;
import net.fcnav.plus.views.FCNavMapLayer.DrawSettings;
...
public class FCNavMapTileView extends SurfaceView implements IMapDownloaderCallback, Callback {
	...
	protected static final Log log = LogUtil.getLog(FCNavMapTileView.class);
	/**MapTree
	...
	private List<FCNavMapLayer> layers = new ArrayList<FCNavMapLayer>();
	
	private BaseMapLayer mainLayer;
	
	private Map<FCNavMapLayer, Float> zOrders = new HashMap<FCNavMapLayer, Float>();
	...
	private final FCNavApplication application;

	public FCNavMapTileView(Context context, AttributeSet attrs) {
		super(context, attrs);
		application = (FCNavApplication) context.getApplicationContext();
		initView();
		
	}

	public FCNavMapTileView(Context context) {
		super(context);
		application = (FCNavApplication) context.getApplicationContext();
		initView();
	}
	...
	public void addLayer(FCNavMapLayer layer, float zOrder) {
		int i = 0;
	...
	public void removeLayer(FCNavMapLayer layer) {
		layers.remove(layer);
		zOrders.remove(layer);
		layer.destroyLayer();
	}

	public List<FCNavMapLayer> getLayers() {
		return layers;
	}
	
	@SuppressWarnings("unchecked")
	public <T extends FCNavMapLayer> T getLayerByClass(Class<T> cl) {
		for(FCNavMapLayer lr : layers) {
			if(cl.isInstance(lr)){
				return (T) lr;
			}
		}
	...
	public int getCenterPointY() {
		if (mapPosition == FCNavSettings.BOTTOM_CONSTANT) {
	...
	protected FCNavSettings settings = null;
	...
	private void drawOverMap(Canvas canvas, RectF latlonRect, RectF tilesRect, DrawSettings drawSettings) {
		...
		for (int i = 0; i < layers.size(); i++) {
			try {
				...
				FCNavMapLayer layer = layers.get(i);
				canvas.save();
				// rotate if needed
		...
		//canvas.restore();  // svk had an exc here 
		if (showMapPosition) {
		...
		public void onZoomEnded(float distance, float relativeToStart) {
			...
			if (application.accessibilityEnabled()) {
				if (newZoom != initialMultiTouchZoom) {
					...
					showMessage(FCNavFormatter.getFormattedDistance((float)MapUtils.getDistance(p1.getLatitude(), p1.getLongitude(), p2.getLatitude(), p2.getLongitude()), getContext()));
				}
			}
		}
