...
public class MapControlsLayer extends FCNavMapLayer implements OnTouchListener, RecognitionListener {
...
//	private Drawable modeShadow;
	...
	//	initBackToMenuButton(view, parent);
		initSpeechRecognitionButton(view, parent);
...
//	private Drawable cacheAppModeIcon = null;
	...
	// svk we do not select it from the map  -> only from settings
	/*
	private void drawApplicationMode(Canvas canvas) {
		...
		if(appMode != cacheApplicationMode){
			...
			} else if(appMode == ApplicationMode.TRUCK){
				cacheAppModeIcon = view.getResources().getDrawable(R.drawable.truck_small);
			} else {
	...
	*/
	...
	private void onApplicationModePress() {
		...
		int[] icons = new int[] { R.drawable.default_mode_small, R.drawable.car_small, R.drawable.bicycle_small, R.drawable.truck_small };
		int[] values = new int[] { R.string.app_mode_default, R.string.app_mode_car, R.string.app_mode_bicycle,
				R.string.app_mode_truck };
	...
	*/
	...
	public boolean onSingleTap(PointF point) {
		//it checks if we pressed Application mode change (Car, Truck, Bicycle)
		/*
	...
	/*
	private void initBackToMenuButton(final FCNavMapTileView view, FrameLayout parent) {
	...
	*/
	private void initRuler(FCNavMapTileView view, FrameLayout parent) {
	...
	private void initZoomButtons(final FCNavMapTileView view, FrameLayout parent) {
	...
	/////////////////  Transparency bar /////////////////////////
	private void initTransparencyBar(final FCNavMapTileView view, FrameLayout parent) {
	...
	private void drawRuler(Canvas canvas) {
		...
		if (view.isZooming()) {
			...
			double roundedDist = FCNavFormatter.calculateRoundedDist(dist * screenRulerPercent, view.getContext());
			
			int cacheRulerDistPix = (int) (pixDensity * roundedDist);
			cacheRulerText = ShadowText.create(FCNavFormatter.getFormattedDistance((float) roundedDist, view.getContext()));
...
//-------------------------------------Speech Recognition stuff--------------------------------------
	...
	private void initSpeechRecognitionButton(final FCNavMapTileView view, FrameLayout parent) {
		android.widget.FrameLayout.LayoutParams params;
		Context ctx = view.getContext();
		backToMenuButton = new Button(ctx);
		backToMenuButton.setContentDescription(ctx.getString(R.string.backToMenu));
		backToMenuButton.setBackgroundResource(R.drawable.record/*R.drawable.map_btn_menu*/);
		params = new FrameLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT,
					Gravity.BOTTOM | Gravity.LEFT);
		parent.addView(backToMenuButton, params);
		backToMenuButton.setEnabled(true);
...
//		modeShadow = view.getResources().getDrawable(R.drawable.zoom_background);
		...
		*/
		//backToMenuButton.setOnTouchListener(this);
		activity.accessibleContent.add(backToMenuButton);
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			this.listening = true;
			this.rec.start();
			//AccessibleToast.makeText(view.getContext(), "ON DOWN", Toast.LENGTH_LONG).show();
			break;
		case MotionEvent.ACTION_UP:
			Date end_date = new Date();
			//AccessibleToast.makeText(view.getContext(), "ON UP", Toast.LENGTH_LONG).show();
	...
	public void onResults(Bundle b) {
		...
		//Log.d(getClass().getName(), hyp);
		if(hyp==null || hyp=="")
			return;
		if(hyp.contains("ROUTE") && (hyp.contains("MY") || hyp.contains("HOUSE")))
		{
			...
			Thread thread = new Thread(){
				...
				public void run() {
					...
					activity.getMapActions().DirectionsFromSpeechCmd();
				}
			};
			thread.start();
			*/
			activity.runOnUiThread(new Runnable() { 
				  @Override
				public void run() { 
				    Toast.makeText(activity, "Prepare Route", Toast.LENGTH_SHORT).show(); 
				    activity.getMapActions().DirectionsFromSpeechCmd();
				  } 
				}); 
		...
		if(hyp.contains("ROUTE") && hyp.contains("CANCEL"))
		{
			activity.runOnUiThread(new Runnable() { 
		    @Override
			public void run() { 
			    
			    activity.getMapActions().CancelRoute();
			  } 
			}); 
