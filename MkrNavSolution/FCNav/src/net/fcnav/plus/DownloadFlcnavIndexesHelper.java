...
import net.fcnav.plus.activities.DownloadIndexActivity.AssetDownloadEntry;
import net.fcnav.plus.activities.DownloadIndexActivity.DownloadEntry;
...
public class DownloadFlcnavIndexesHelper {
	private final static Log log = LogUtil.getLog(DownloadFlcnavIndexesHelper.class);
	...
	public static IndexFileList getIndexesList(Context ctx) {
		...
		listVoiceAssets(result, amanager, pm, ((FCNavApplication) ctx.getApplicationContext()).getSettings());
		return result;
	}
	
	private static void listVoiceAssets(IndexFileList result, AssetManager amanager, PackageManager pm, 
			FCNavSettings settings) {
		String[] list;
		try {
			String ext = IndexItem.addVersionToExt(IndexConstants.TTSVOICE_INDEX_EXT_ZIP, IndexConstants.TTSVOICE_VERSION);
			File voicePath = settings.extendFlcnavPath(ResourceManager.VOICE_PATH);
			list = amanager.list("voice");
			String date = "";
			long dateModified = System.currentTimeMillis();
			try {
				ApplicationInfo appInfo = pm.getApplicationInfo(FCNavApplication.class.getPackage().getName(), 0);
	...
	private static IndexFileList downloadIndexesListFromInternet(String versionAsUrl){
		try {
			...
			try {
				URL url = new URL("http://download.fcnav.net/get_indexes?" + versionAsUrl ); //$NON-NLS-1$
				...
				while((next = parser.next()) != XmlPullParser.END_DOCUMENT) {
					...
					} else if (next == XmlPullParser.START_TAG && ("flcnav_regions".equals(parser.getName()))) {
...
//				}
				...
				//s = ctx.getString(R.string.voice_provider_descr);//svk
...
//			}
		...
		public DownloadEntry createDownloadEntry(Context ctx) {
			...
			FCNavSettings settings = ((FCNavApplication) ctx.getApplicationContext()).getSettings();
			if(fileName.endsWith(IndexConstants.BINARY_MAP_INDEX_EXT)){
				parent = settings.extendFlcnavPath(ResourceManager.APP_DIR);
				...
				parent = settings.extendFlcnavPath(ResourceManager.VOICE_PATH);
			...
			if(parent == null || !parent.exists()){
				...
				File backup = settings.extendFlcnavPath(ResourceManager.BACKUP_PATH + entry.fileToUnzip.getName());
				if (backup.exists()) {
