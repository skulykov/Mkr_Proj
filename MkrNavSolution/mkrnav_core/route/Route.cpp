...
void searchRouteInternal(RoutingContext* ctx, SHARED_PTR<RouteSegment> start, SHARED_PTR<RouteSegment> end, bool leftSideNavigation) {
	...
	if(ctx->config.initialDirection > -180 && ctx->config.initialDirection < 180) {
		...
		if(abs(MapUtils::alignAngleDifference(diff)) <= M_PI / 3) {
			ctx->firstRoadDirection = 1;
		} else if(abs(MapUtils::alignAngleDifference(diff - M_PI )) <= M_PI / 3) {
			ctx->firstRoadDirection = -1;
		}
	...
	flcnav_log_print(SK_MESSAGE_WARNING, "[Native] Result visited (visited roads %d, visited segments %d / %d , queue sizes %d / %d ) ",
	...
	flcnav_log_print(SK_MESSAGE_WARNING, "[Native] Result timing (time to load %d, time to calc %d, loaded tiles %d) ", ctx->timeToLoad.getElapsedTime()
	...
	flcnav_log_print(SK_MESSAGE_WARNING, "[Native] Memory occupied (Routing context %d Kb, search %d Kb)", ctx->getSize()/ 1024, sz/1024);
}
...
bool processRouteSegment(RoutingContext* ctx, bool reverseWaySearch, SEGMENTS_QUEUE& graphSegments,
	...
	if(TRACE_ROUTING) {
		flcnav_log_print(SK_MESSAGE_DEBUG, "Process segment id=%lld name=%s dist=%f", road->id, road->getName().c_str(), segment->distanceFromStart);
	}
	...
	while (hasNext) {
		...
		if (!alreadyVisited) {
			...
			if (next->parentRoute.get() == NULL
				...
				if(TRACE_ROUTING) {
					flcnav_log_print(SK_MESSAGE_DEBUG, ">> next  segment id=%lld name=%s dist=%f", next->road->id, next->road->getName().c_str(), next->distanceFromStart);
				}
...
void attachConnectedRoads(RoutingContext* ctx, vector<RouteSegmentResult>& res) {
	...
	for (; it != res.end(); it++) {
		...
		do {
			SHARED_PTR<RouteSegment> s = ctx->loadRouteSegment(it->road->pointsX[j], it->road->pointsY[j]);
...
vector<RouteSegmentResult> convertFinalSegmentToResults(RoutingContext* ctx) {
	...
	if (ctx->finalRouteSegment.get() != NULL) {
		LOGI( "Routing calculated time distance %f", ctx->finalRouteSegment->distanceFromStart);
...
vector<RouteSegmentResult> searchRouteInternal(RoutingContext* ctx, bool leftSideNavigation) {
	...
	if(start.get() == NULL) {
		flcnav_log_print(SK_MESSAGE_WARNING, "Start point was not found [Native]");
		if(ctx->progress.get()) {
			ctx->progress->setSegmentNotFound(0);
		}
		return vector<RouteSegmentResult>();
	} else {
		flcnav_log_print(SK_MESSAGE_WARNING, "Start point was found %lld [Native]", start->road->id);
	}
	...
	if(end.get() == NULL) {
		...
		flcnav_log_print(SK_MESSAGE_WARNING, "End point was not found [Native]");
		return vector<RouteSegmentResult>();
	} else {
		flcnav_log_print(SK_MESSAGE_WARNING, "End point was found %lld [Native]", end->road->id);
	}
	searchRouteInternal(ctx, start, end, leftSideNavigation);
	vector<RouteSegmentResult> res = convertFinalSegmentToResults(ctx);
	
	if(!RouteP::validateAllPointsConnected(res))
	{
     //   return MkrMap::RouteCalculationResult("Calculated route has broken paths");
	}
	attachConnectedRoads(ctx, res);

	//splitRoadsAndAttachRoadSegments(context, route);

    RouteP::calculateTimeSpeedInRoute(/*context,*/ res);

    RouteP::addTurnInfoToRoute(leftSideNavigation, res);
	return res;
}
