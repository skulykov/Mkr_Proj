#ifndef _MKR_BINARY_ROUTE_H
#define _MKR_BINARY_ROUTE_H
...
typedef unordered_map<string, float> MAP_STR_FLOAT;
typedef unordered_map<string, string> MAP_STR_STR;

static double measuredDist(int x1, int y1, int x2, int y2) {
	return MapUtils::getDistance(get31LatitudeY(y1), get31LongitudeX(x1), get31LatitudeY(y2),
			get31LongitudeX(x2));
}
...
struct RouteSegmentResult {
	SHARED_PTR<RouteDataObject> road;  // should be road
	int startPointIndex;
	int endPointIndex;
	int time;
	float speed,distance;
	TurnInfo turnInfo;//_turnType;
	string description;
	vector<vector<RouteSegmentResult> > attachedRoutes;
	RouteSegmentResult(SHARED_PTR<RouteDataObject> road, int startPointIndex, int endPointIndex) :
		road(road), startPointIndex(startPointIndex), endPointIndex (endPointIndex) {
	...
	{
		return road->getDirectionDelta(startPointIndex, startPointIndex < endPointIndex) / M_PI * 180.0;
	}

	double getBearingEnd() const
	{
		return MapUtils::normalizedAngleRadians(road->getDirectionDelta(endPointIndex, startPointIndex > endPointIndex) - M_PI) / M_PI * 180.0;
	}
};
...
struct RoutingSubregionTile {
	...
	int loaded;
	int size ;
	unordered_map<int64_t, SHARED_PTR<RouteSegment> > routes;
	...
	void add(SHARED_PTR<RouteDataObject> o) {
		...
		//size += o->getSize() + sizeof(RouteSegment)* 1;//o->pointsX.size();
		//RouteSegment *segment =  new RouteSegment(o, 0);
		//routes[o->id]=segment;
...
struct RoutingConfiguration {
	...
	bool acceptLine(SHARED_PTR<RouteDataObject> r) {
		...
		for(; t != r->types.end(); t++) {
			tag_value type = r->region->decodingRules[*t];
			MAP_STR_FLOAT::iterator it=highwaySpeed.find(type.second);
			if(type.first=="highway" && it!=highwaySpeed.end())
			{
				accepted = true;
				break;
			}
			...
			//the code below insert the new item into highwaySpeed if it does not exist
			// I do not waant this !!!!!!!!
			/*if(type.first=="highway" && highwaySpeed[type.second] > 0) {
				accepted = true;
				break;
	...
	bool acceptLine(SHARED_PTR<RouteDataObject> r) {
		std::vector<uint32_t>::iterator t = r->types.begin();
		bool accepted = false;
		/*unordered_map<int, std::string >::const_iterator it;
		for(it=r->names.begin();it!=r->names.end();++it)
		{
			if(it->second=="Lauderdale Drive")
			{
				accepted = false;
			}
		}*/
	...
	float defineSpeedPriority(SHARED_PTR<RouteDataObject> r) {
		...
		for(; t != r->types.end(); t++) {
			tag_value type = r->region->decodingRules[*t];
			if(highwayPriorities.find(type.first+"$"+type.second) != /*highwaySpeed*/highwayPriorities.end()) {
...
struct RoutingContext {
	typedef unordered_map<int64_t, SHARED_PTR<RoutingSubregionTile> > MAP_SUBREGION_TILES;
	...
	unordered_map<int64_t, std::vector<SHARED_PTR<RoutingSubregionTile> > > indexedSubregions;
	...
	void unloadUnusedTiles(int memoryLimit) {
		...
		LOGI( "Run GC (before %f Mb after %f Mb) unload %d of %d tiles",
	...
	void loadTileData(int x31, int y31, int zoomAround, vector<SHARED_PTR<RouteDataObject> >& dataObjects ) {
		...
		for(int i = -t; i <= t; i++) {
			for(int j = -t; j <= t; j++) {
				...
				for(int j = 0; j<subregions.size(); j++) {
					if(subregions[j]->isLoaded()) {
						unordered_map<int64_t, SHARED_PTR<RouteSegment> >::iterator s = subregions[j]->routes.begin();
	...
	SHARED_PTR<RouteSegment> loadRouteSegment(int x31, int y31) {
		...
		loadHeaders(xloc, yloc);
		vector<SHARED_PTR<RoutingSubregionTile> >& subregions = indexedSubregions[tileId];
		unordered_map<int64_t, SHARED_PTR<RouteDataObject> > excludeDuplications;
...
#endif /*_MkrMap_BINARY_ROUTE_PLANNER_H*/
