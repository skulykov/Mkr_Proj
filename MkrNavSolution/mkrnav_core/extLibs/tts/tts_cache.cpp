...
//static Sk_ConfigDescriptor RMConfigTTSCacheEnabled =
//      SK_CONFIG_ITEM( TTS_CFG_CATEGORY, TTS_CFG_CACHE_ENABLED );
//static Sk_ConfigDescriptor RMConfigTTSDbVersion =
//      SK_CONFIG_ITEM( TTS_CFG_CATEGORY, TTS_CFG_DB_VERSION );
...
{
   /*sk_config_declare("preferences", &RMConfigTTSCacheEnabled,
            TTS_CFG_CACHE_ENABLED_DEFAULT, NULL);

   sk_config_declare("session", &RMConfigTTSDbVersion, "0", NULL);*/

   sgTtsCache.hash = sk_hash_new( "TTS CACHE", TTS_CACHE_SIZE );
}
...
{
   int enabled = 1;//!strcmp( sk_config_get( &RMConfigTTSCacheEnabled ), "yes" );
   return enabled;
}
...
TtsCacheEntry* _cache_set_entry( int slot, const char* text, const TtsData* tts_data, const TtsPath* tts_path )
{
   ...
   sk_log( SK_DEBUG, "Adding text '%s' to the cache. Slot: %d", text, slot );
   ...
   cache_entry->key = sk_hash_string( text );

   sk_hash_add( sgTtsCache.hash, cache_entry->key, slot );
...
#ifdef TTS_CACHE_DEBUG
     sk_log( SK_DEBUG, "The text '%s' is already in the cache. Slot: %d", slot );
#endif // TTS_CACHE_DEBUG
...
void tts_cache_remove( const char* text, const char* voice_id, TtsDbDataStorageType storage_type )
{
   ...
   // AGA DEBUG - Reduce level
   sk_log( SK_WARNING, TTS_LOG_STR( "Removing entry from the database: (%s, %s)" ),
         text, voice_id );
...
BOOL tts_cache_get( const char* text, const char* voice_id, TtsData* cached_data, TtsPath* cached_path )
{
   ...
   if ( slot >= 0 )
   {
      sk_log( SK_DEBUG, "The text '%s' is found in the cache in slot: %d", text, slot );
   }
   else
   {
      sk_log( SK_DEBUG, "The text '%s' is not found in the cache", text );
   }
#endif // TTS_CACHE_DEBUG
...
static int _find_entry( const char* text, TtsCacheContext* context )
{
   ...
   key = sk_hash_string( text );

   slot = sk_hash_get_first( context->hash, key );
   ...
   while (slot >= 0)
   {
     ...
     slot = sk_hash_get_next( context->hash, slot );
   }
...
#ifdef TTS_CACHE_DEBUG
   sk_log( SK_DEBUG, "Removing entry with text '%s' from the cache. Slot: %d", cache_entry->text, slot );
#endif // TTS_CACHE_DEBUG

   sk_hash_remove( context->hash, cache_entry->key, slot );
...
static void _check_db_version( void )
{
   ...
   /*if ( sk_config_get_integer( &RMConfigTTSDbVersion ) < TTS_DB_VERSION )
   {
      tts_db_clear( sgTtsDbType, NULL );
      sk_config_set_integer( &RMConfigTTSDbVersion, TTS_DB_VERSION );
   }*/
}
