...
BOOL tts_db_sqlite_store( const TtsDbEntry* entry, TtsDbDataStorageType storage_type, const TtsData* db_data, const TtsPath* db_path )
{
   ...
   if ( !db )
   {
      sk_log( SK_ERROR, "TTS storage failed - cannot open database" );
      return FALSE;
   }
   /*
    * Prepare the string
   ...
   sk_log( SK_DEBUG, "TTS SQLite Store. Query string: %s", stmt_string );
...
BOOL tts_db_sqlite_remove( const TtsDbEntry* entry )
{
   ...
   if ( !db )
   {
      sk_log( SK_ERROR, "TTS cache remove failed - cannot open database" );
   }
...
BOOL tts_db_sqlite_get_info( const TtsDbEntry* entry, TtsTextType* text_type, TtsDbDataStorageType* storage_type, TtsPath* db_path )
{
   ...
   if ( !db )
   {
      sk_log( SK_ERROR, "Tile loading failed - cannot open database" );
      return FALSE;
   }
   ...
   if ( ret_val == SQLITE_ROW )
   {
      // Storage type
      if ( storage_type )
         *storage_type = (TtsDbDataStorageType)sqlite3_column_int( stmt, 0 );
      // Path data
      const char* path_data = (const char*)sqlite3_column_text( stmt, 1 );
      if ( db_path )
      {
         if ( path_data )
         {
      ...
      // Text type
      if ( text_type )
         *text_type = (TtsTextType)sqlite3_column_int( stmt, 2 );
   }
   else
   {
      res = FALSE;
...
BOOL tts_db_sqlite_get( const TtsDbEntry* entry, TtsDbDataStorageType* storage_type, TtsData *db_data, TtsPath* db_path )
{
   ...
   if ( ret_val == SQLITE_ROW )
   {
      ...
      if ( storage_type )
      {
         *storage_type =(TtsDbDataStorageType) sqlite3_column_int( stmt, 1 );
      }

      if ( db_path )
      {
         path_data = (const char*)sqlite3_column_text( stmt, 2 );
         if ( path_data )
         {
...
BOOL tts_db_sqlite_destroy_voice( const char* voice_id )
{
   ...
   sk_log( SK_DEBUG, "TTS SQLite Destroy voice. Query string: %s. Voice: %s", stmt_string, SAFE_STR( voice_id ) );
...
static BOOL _check_sqlite_error_line( const char* errstr, int code, int line )
{
   ...
   if ( code != SQLITE_OK )
   {
      ...
      sk_log( SK_MESSAGE_ERROR, __FILE__, line, "SQLite error in %s executing sqlite statement. Error : %d ( %s )",
            errstr, code, errmsg );
      return FALSE;
   }
   return TRUE;
...
static const char* _get_db_file( void )
{
   ...
   if ( !full_path[0] )
   {
      const char *path = sk_path_tts();
      sk_path_format( full_path, sizeof( full_path ), path, TTS_DB_SQLITE_FILE_NAME );
   }
   return full_path;
}
...
static sqlite3* _get_db( void )
{
   ...
   if ( !full_path[0] )
   {
      sk_log( SK_ERROR, "Can't define the database filename" );
      return sgSQLiteDb;
   }
...
static sqlite3*  _trans_open( const char* table_name )
{
   ...
   sk_log( SK_DEBUG, "Transaction open %d, %d ", sgIsInTransaction, sgTransStmtsCount );
   if ( sgIsInTransaction )
   {
      sgTransStmtsCount++;
      if ( sgTransStmtsCount >= TTS_DB_SQLITE_TRANS_STMTS_CNT )
      {
         sk_log( SK_DEBUG, "Transaction statements number exceeded - committing" );
         sk_main_remove_periodic( _trans_timeout );
         _trans_commit();
      }
      else
      {
   ...
   if ( db != NULL  )
   {
      ...
      sgTransStmtsCount = 0;
      sk_main_set_periodic( TTS_DB_SQLITE_TRANS_TIMEOUT, _trans_timeout );
   }
   else
   {
      sk_log( SK_ERROR, "Begin transaction failed - cannot open database" );
   }
...
//   int ret_val;
   ...
   if ( !sgSQLiteDb )
   {
      sk_log( SK_ERROR, "Commit transaction failed - cannot open database" );
      return;
   }
   check_sqlite_error( "Commit transaction", sqlite3_exec( sgSQLiteDb, "COMMIT;", NULL, NULL, NULL) );

   sk_log( SK_DEBUG, "Transaction committed %d, %d", sgIsInTransaction, sgTransStmtsCount );
...
//   int ret_val;
   ...
   if ( !sgSQLiteDb )
   {
      sk_log( SK_ERROR, "Rollback transaction failed - cannot open database" );
      return;
   }
...
static void _trans_timeout( void )
{
   sk_log( SK_DEBUG, "Transaction timeout expired - committing" );
   if ( sgIsInTransaction )
   {
      sk_main_remove_periodic( _trans_timeout );
      _trans_commit();
