...
BOOL tts_db_get_path( const TtsDbEntry* entry, TtsPath* db_path )
{
   ...
   if ( !result )
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Error retrieving the path!" ) );
   }
   if ( !( storage_type & __tts_db_data_storage__file ) )
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Request for path for the non fs storage record!" ) );
   }
...
void tts_db_generate_path( const char* voice_id, TtsPath* db_path )
{
   ...
   const EpochTimeMicroSec* time_now = Sk_CTime::sk_time_get_epoch_us( NULL );
   ...
   // Full path
   sk_path_format( db_path->path, TTS_PATH_MAXLEN, sk_path_tts(), path_suffix );

   if ( strcmp( s_voice_id, voice_id ) ) // Save io time
   {
      char* parent = sk_path_parent( db_path->path, NULL );
...
void tts_db_clear( TtsDbDataStorageType storage_type, const char* voice_id )
{
   ...
   if ( storage_type & __tts_db_data_storage__file )
   {
      ...
      if ( voice_id )
         snprintf( path, sizeof( path ), "%s//%s//%s", sk_path_tts(), TTS_DB_FILES_ROOT_DIR, voice_id );
      else
         snprintf( path, sizeof( path ), "%s//%s", sk_path_tts(), TTS_DB_FILES_ROOT_DIR );

      sk_log( SK_INFO, "Removing the TTS fs database at %s", path );
      sk_file_rmdir( path, NULL );
   }
}
