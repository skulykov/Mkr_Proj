/* tts.cpp - implementation of the Text To Speech (TTS) interface layer.
...
#define _TEST_TTS_ 1
#if _TEST_TTS_
BOOL Sk_CSound::sgInitialized=FALSE; //initialization must be done only once
//======================== Local defines ========================
...
//svk config is not defined!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//static RoadMapConfigDescriptor RMConfigTTSEnabled =
//      SK_CONFIG_ITEM( TTS_CFG_CATEGORY, TTS_CFG_FEATURE_ENABLED );
//static RoadMapConfigDescriptor RMConfigTTSVoiceId =
//      SK_CONFIG_ITEM( TTS_CFG_CATEGORY, TTS_CFG_VOICE_ID );
//static RoadMapConfigDescriptor RMConfigTTSDefaultVoiceId =
//      SK_CONFIG_ITEM( TTS_CFG_CATEGORY, TTS_CFG_DEFAULT_VOICE_ID );
...
void tts_initialize( void )
{
   ...
   {
      //sk_config_set( &RMConfigTTSVoiceId, sgTtsDefaultVoiceId );
   ...
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Provider is not registered for current voice id: %s." ), sgTtsVoiceId );
      sgTtsVoiceId = sgTtsDefaultVoiceId;
      if ( !( sgActiveProvider = _voice_service_provider( sgTtsVoiceId ) ) )
      {
         sk_log( SK_ERROR, TTS_LOG_STR( "Critical TTS Engine error. Provider is not registered for default voice id: %s." ), sgTtsVoiceId );
         return;
      }
   }
...
void tts_load_config( void )
{
   ...
   if ( !initialized )
   {
       ...
       */
      /*sk_config_declare("preferences", &RMConfigTTSEnabled,
            TTS_CFG_FEATURE_ENABLED_DEFAULT, NULL);
      sk_config_declare("preferences", &RMConfigTTSDefaultVoiceId,
            TTS_CFG_VOICE_ID_DEFAULT, NULL);
      sk_config_declare( "user", &RMConfigTTSVoiceId, TTS_CFG_VOICE_ID_NOT_DEFINED, NULL );*/
      initialized = TRUE;
   }
    ...
    */
   /*sgTtsDefaultVoiceId = sk_config_get( &RMConfigTTSDefaultVoiceId );
   sgTtsFeatureEnabled = !strcmp( "yes", sk_config_get( &RMConfigTTSEnabled ) );
   sgTtsVoiceId = sk_config_get( &RMConfigTTSVoiceId );*/
}
...
void tts_set_voice( const char* voice_id )
{
   ...
   //sk_log( SK_INFO, TTS_LOG_STR( "Voice change request: %s => %s" ), SAFE_STR( sgTtsVoiceId ), SAFE_STR( voice_id ) );

   // Find active provider
   if ( !( provider = _voice_service_provider( voice_id ) ) )
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Critical TTS Engine error. Provider is not registered for voice id: %s." ), sgTtsVoiceId );
      return;
   }

   if ( !TTS_VOICE_VALID( tts_voices_get( voice_id, NULL ) ) )
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Current voice %s is not valid. Server may not support the requests" ), voice_id );
   }
   ...
   //sk_config_set( &RMConfigTTSVoiceId, sgTtsVoiceId );
...
void tts_request_ex( const char* text, TtsTextType text_type, TtsRequestCompletedCb completed_cb, void* user_context, int flags )
{
   ...
   if ( !tts_enabled() )
   {
      sk_log(SK_WARNING, TTS_LOG_STR( "TTS is disabled cannot post synthesize request for %s." ), text );
      return;
   }
   ...
   if ( !text || !text[0] )
   {
      ...
      sk_log( SK_WARNING, TTS_LOG_STR( "NULL or empty text cannot be requested!" ) );
      return;
   }
...
BOOL tts_register_provider( const TtsProvider* tts_provider )
{
   ...
   for( i = 0; i < TTS_PROVIDERS_MAX_NUM; ++i )
   {
      if ( !sgTtsProviders[i].registered )
      {
         ...
         sk_log( SK_INFO, TTS_LOG_STR( "Provider %s was registered successfully" ), tts_provider->provider_name );
         break;
      }
   }
   if ( i == TTS_PROVIDERS_MAX_NUM )
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Cannot register more providers. Maximum: %d" ), TTS_PROVIDERS_MAX_NUM );
      result = FALSE;
   }
   return result;
}
...
BOOL tts_update_provider( const TtsProvider* tts_provider )
{
   ...
   for( i = 0; i < TTS_PROVIDERS_MAX_NUM; ++i )
   {
            ...
            !strcmp( sgTtsProviders[i].provider_name, tts_provider->provider_name ) )
      {
         ...
         sk_log( SK_INFO, TTS_LOG_STR( "Provider %s was updated successfully" ), tts_provider->provider_name );
         break;
      }
   }
   if ( i == TTS_PROVIDERS_MAX_NUM )
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Cannot find registered provider with name %s" ), tts_provider->provider_name );
      result = FALSE;
   }
   else
   {
      ...
      if ( sgTtsVoiceId )
      {
         ...
         {
            sk_log( SK_WARNING, TTS_LOG_STR( "Voice %s is invalidated. Trying to set the default one" ), voice->voice_id );
            voice_id = sgTtsDefaultVoiceId;
         }
      }
      else
      {
         sk_log( SK_WARNING, TTS_LOG_STR( "There is no voice defined. Setting the default one" ) );
         voice_id = sgTtsDefaultVoiceId;
      }
...
BOOL tts_commit( void )
{
   ...
   if ( !tts_enabled() )
   {
      sk_log(SK_WARNING, TTS_LOG_STR( "TTS is not enabled. Cannot commit synthesize requests. Feature: %d, Provider: %d" ),
   ...
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Overflow in concurrent requests for the provider. Maximum: %d" ),
   ...
   if ( idx_list_count == 0 )
   {
      sk_log( SK_DEBUG, TTS_LOG_STR( "There are no strings to commit." ) );
      return TRUE;
   }
   ...
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Error allocating provider context!" ) );
      return FALSE;
   }
   provider_request->start_time = Sk_CTime::sk_time_get_millis();
   ...
   sk_log( SK_DEBUG, TTS_LOG_STR( "Committing list of %d tts strings." ), i );
...
void tts_set_feature_enabled( BOOL value )
{
   ...
   /*if ( sgTtsFeatureEnabled )
      sk_config_set( &RMConfigTTSEnabled, "yes" );
   else
      sk_config_set( &RMConfigTTSEnabled, "no" );*/
}
...
void tts_register_on_voice_changed( TtsOnVoiceChangedCb cb )
{
   ...
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Maximum number of callbacks (on voice changed) was exceeded" ) );
   }
   else
   {
...
BOOL tts_register_fiter_prefix( const char* prefix )
{
   ...
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Error adding the prefix: %s. Current number: %d(%d)" ),
...
TtsPlayList tts_playlist_create( const char* voice_id )
{
   ...
   if ( tts_enabled() )
   {
      ...
      switch ( provider->storage_type )
      {
         ...
         {
            sound_list = Sk_CSound::sk_sound_list_create( 0 );
            break;
         }
         case __tts_db_data_storage__blob_record:
         {
            sound_list = Sk_CSound::sk_sound_list_create( SOUND_LIST_BUFFERS );
            break;
         }
         default:
         {
            sk_log( SK_WARNING, "Data storage type %d is not supported" );
         }
      }
      if ( sound_list )
      {
         play_list = (TtsPlayList)malloc( sizeof( TtsPlayList ) );
...
BOOL tts_playlist_add( TtsPlayList list, const char* text )
{
   ...
   if ( !text )
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Cannot add NULL texts to the playlist" ) );
      return FALSE;
   }
   ...
   switch ( provider->storage_type )
   {
      case __tts_db_data_storage__file:
      {
         ...
         {
            result = Sk_CSound::sk_sound_list_add( list->sound_list, path.path );
            sk_log( SK_DEBUG, TTS_LOG_STR( "Adding text %s (Voice: %s). Path: %s to the playlist. Status: %d" ),
            ...
            if ( result == SND_LIST_ERR_NO_FILE )
            {
               ...
               sk_log( SK_WARNING, TTS_LOG_STR( "File %s doesn't exist!!!. Removing the entry!" ), path.path );
      ...
      case __tts_db_data_storage__blob_record:
      {
         ...
         if ( tts_cache_get( _parse_text( text ), list->voice_id, &data, NULL ) )
            result = Sk_CSound::sk_sound_list_add_buf( list->sound_list, data.data, data.data_size );
         break;
      }
      default:
      {
...
BOOL tts_playlist_play( TtsPlayList list )
{
   ...
   result = Sk_CSound::sk_sound_play_list( list->sound_list );
 ...
 ******************************************************************************
 */
Sk_SoundList tts_playlist_export_list( TtsPlayList list, BOOL free_tts_playlist )
{
   Sk_SoundList sl = list->sound_list;
...
{
   Sk_CSound::sk_sound_list_free( list->sound_list );
   free( list );
}
...
static void _on_response( void* context, int res_status, TtsSynthResponseData* response_data )
{
   ...
   unsigned int delta = Sk_CTime::sk_time_get_millis() - provider_request->start_time;
   int matched_count;

   sk_log( SK_DEBUG, TTS_LOG_STR( "Request was completed in %d ms. Status: %d" ), delta, res_status );
   ...
   if ( res_status == TTS_RES_STATUS_SUCCESS )
   {
      ...
      sk_log( SK_DEBUG, TTS_LOG_STR( "Received %d elements in tts response. Matched: %d. Requested: %d" ),
            response_data->count, matched_count, provider_request->idx_list_count );

   }
   else
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Error in response" ) );
...
static int _process_received( TtsProviderRequest* provider_request, TtsSynthResponseData* response_data )
{
    ...
    for ( i = 0; i < provider_request->idx_list_count; ++i )
    {
       ...
       {
          sk_log( SK_ERROR, TTS_LOG_STR( "Internal error. Current status %d differs from 'COMMITTED' (%d)" ),
       ...
       if ( j == TTS_BATCH_REQUESTS_LIMIT )
       {
          ...
          sk_log( SK_ERROR, TTS_LOG_STR( "The text %s was not synthesized." ), queued_text );
       ...
       else
       {
          ...
          sk_log( SK_DEBUG, TTS_LOG_STR( "Successfully received audio for text: %s. Voice: %s. Path: %s" ),
   ...
   if ( tts_request->flags & TTS_FLAG_RETRY )
   {
      if ( tts_request->retry_index < TTS_RETRY_COUNT )
      {
         ...
         *status |= TTS_RES_STATUS_RETRY_ON;
         ...
         sk_log( SK_WARNING, TTS_LOG_STR( "Applying retry #%d ( %d ) for text: %s" ), tts_request->retry_index, TTS_RETRY_COUNT, tts_request->text );
      }
      else
      {
         *status |= TTS_RES_STATUS_RETRY_EXHAUSTED;
         sk_log( SK_WARNING, TTS_LOG_STR( "Retries number exhausted #%d ( %d ) for text: %s. Giving up ... " ), tts_request->retry_index, TTS_RETRY_COUNT, tts_request->text );
      }
   }
...
static BOOL _process_cached( const TtsRequest* request_ctx )
{
   ...
   sk_log( SK_DEBUG, TTS_LOG_STR( "Processing cached element: %s" ), request_ctx->text );
...
static int _process_error( const TtsProviderRequest* provider_request )
{
   ...
   for ( i = 0, count = 0; i < provider_request->idx_list_count; ++i )
   {
      ...
      sk_log( SK_WARNING, TTS_LOG_STR( "Error reported for text: %s" ), tts_request->text );
...
static void _queue_one( const char* text, TtsTextType text_type, TtsRequestCompletedCb completed_cb, void* user_context, int flags )
{
   ...
   sk_log( SK_DEBUG, TTS_LOG_STR( "Adding text %s to the queue!" ), text );

   if ( text == NULL )
   {
      sk_log( SK_ERROR, TTS_LOG_STR( "Cannot prepare NULL text." ) );
      return;
   }

   queue_idx = tts_queue_get_index( text );
   if (queue_idx >= 0) {
      TtsRequest* request = (TtsRequest*)tts_queue_get_context( queue_idx );
...
static TtsProviderRequest* _provider_ctx_allocate( void )
{
   ...
   {
      sk_log( SK_ERROR, "The TTS provider request pool is full!" );
   }
   return ctx;
}
...
static void _voice_updated( const TtsVoice* new_voice, const TtsVoice* old_voice )
{
   ...
   if ( old_voice )
   {
      ...
      {
         sk_log( SK_WARNING, TTS_LOG_STR( "Voice: %s version is incremented ( %d =>> %d ). Clearing the database. Current voice: %s" ),
...
static void _add_request_callback( TtsRequest* request, TtsRequestCompletedCb cb, void* cb_ctx )
{
   ...
   {
      sk_log( SK_WARNING, TTS_LOG_STR( "Can't register more than %d callbacks for text: %s" ), TTS_COMPLETED_CB_MAX_NUM, request->text );
      return;
   }
   if ( cb )
   {
