...
#include "tts_defs.h"
#include "../net/sk_httpcopy_async.h"
#include "../net/websvc/web_date_format.h"
...
{
   TtsVoicePtr tts_voice = (TtsVoicePtr)malloc( sizeof( *tts_voice ) );
...
//            break;
         ...
         case __tts_voice_field__code:
            ...
            break;
         default:
            sk_log( SK_WARNING, "Invalid field index in the CSV file: %d", field_index );
      }
      field_index++;
   ...
   {
      sk_log( SK_ERROR, "Incomplete line: %s. Only %d fields found", line, field_index );
      free( tts_voice );
      tts_voice = NULL;
   }
   return tts_voice;
...
static void _voices_load( const char* service_provider, const char* path )
{
   ...
   if ( !sk_file_exists( path, NULL )  )
   {
      sk_log( SK_WARNING, "TTS voices configuration (%s) does not exist!", path );
      return;
   }

   file = sk_file_fopen( path, NULL, "r" );
   ...
   if ( file )
   {
      ...
      if ( initial_load )
         sgTtsVoices = (TtsVoicePtr*)malloc( sizeof( TtsVoicePtr ) * voices_alloc_count );
      ...
      while ( !feof( (FILE*) file ) )
      {
          ...
          if ( tts_voice != NULL )
          {
             ...
             else // New entry
             {
                ...
                if ( sgTtsVoicesCount == voices_alloc_count )
                {
                   ...
                   sgTtsVoices = (TtsVoicePtr*)realloc( sgTtsVoices, voices_alloc_count );
                }
          ...
          {
             sk_log( SK_ERROR, "Error reading line %s in file: %s", line, path );
          }
      } // while
...
static const char* tts_voices_path( void )
{
   ...
   if ( path[0] == 0 )
   {
      sk_path_format( path, sizeof( path ), sk_path_tts(), TTS_VOICES_CONFIG_FILE_NAME );
   }
   return path;
}
