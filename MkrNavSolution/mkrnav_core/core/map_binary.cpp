...
#include "proto/flcnav_index.pb.h"
...
FlcNavStoredIndex* cache = NULL;
typedef unordered_set<long long> IDS_SET;
...
bool readMapTreeBounds(CodedInputStream* input, MapTreeBounds* tree, MapRoot* root) {
	...
	while ((tag = input->ReadTag()) != 0) {
		...
		case FlcNavMapIndex_MapDataBox::kLeftFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &si)));
			tree->left = si + root->left;
			break;
		}
		case FlcNavMapIndex_MapDataBox::kRightFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &si)));
			tree->right = si + root->right;
			break;
		}
		case FlcNavMapIndex_MapDataBox::kTopFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &si)));
			tree->top = si + root->top;
			break;
		}
		case FlcNavMapIndex_MapDataBox::kBottomFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &si)));
			tree->bottom = si + root->bottom;
			break;
		}
		case FlcNavMapIndex_MapDataBox::kOceanFieldNumber: {
...
bool readMapLevel(CodedInputStream* input, MapRoot* root, bool initSubtrees) {
	...
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		case FlcNavMapIndex_MapRootLevel::kMaxZoomFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &root->maxZoom)));
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kMinZoomFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &root->minZoom)));
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kBottomFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &si)));
			root->bottom = si;
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kTopFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &si)));
			root->top = si;
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kLeftFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &si)));
			root->left = si;
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kRightFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_INT32>(input, &si)));
			root->right = si;
			break;
		}
		case FlcNavMapIndex_MapRootLevel::kBoxesFieldNumber: {
			if (!initSubtrees) {
		...
		case FlcNavMapIndex_MapRootLevel::kBlocksFieldNumber: {
			input->Skip(input->BytesUntilLimit());
			break;
		}
...
bool readRouteEncodingRule(CodedInputStream* input, RoutingIndex* index, uint32_t id) {
	...
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		case FlcNavRoutingIndex_RouteEncodingRule::kValueFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &value)));
			break;
		}
		case FlcNavRoutingIndex_RouteEncodingRule::kTagFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &tagS)));
			break;
		}
		case FlcNavRoutingIndex_RouteEncodingRule::kIdFieldNumber: {
...
bool readMapEncodingRule(CodedInputStream* input, MapIndex* index, uint32_t id) {
	...
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		case FlcNavMapIndex_MapEncodingRule::kValueFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &value)));
			break;
		}
		case FlcNavMapIndex_MapEncodingRule::kTagFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &tagS)));
			break;
		}
		case FlcNavMapIndex_MapEncodingRule::kTypeFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &type)));
			break;
		}
		case FlcNavMapIndex_MapEncodingRule::kIdFieldNumber: {
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &id)));
			break;
		}
		default: {
...
bool readRouteTree(CodedInputStream* input, RouteSubregion* thisTree, RouteSubregion* parentTree, RoutingIndex* ind,
	...
	while ((tag = input->ReadTag()) != 0) {
		...
		case FlcNavRoutingIndex_RouteDataBox::kLeftFieldNumber: {
		...
		case FlcNavRoutingIndex_RouteDataBox::kRightFieldNumber: {
			WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &i);
			if (readCoordinates) {
		...
		case FlcNavRoutingIndex_RouteDataBox::kTopFieldNumber: {
			WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &i);
			if (readCoordinates) {
		...
		case FlcNavRoutingIndex_RouteDataBox::kBottomFieldNumber: {
			WireFormatLite::ReadPrimitive<int32_t, WireFormatLite::TYPE_SINT32>(input, &i);
			if (readCoordinates) {
		...
		case FlcNavRoutingIndex_RouteDataBox::kShiftToDataFieldNumber: {
			readInt(input, &thisTree->mapDataBlock);
			break;
		}
		case FlcNavRoutingIndex_RouteDataBox::kBoxesFieldNumber: {
			if (readChildren) {
...
bool readRoutingIndex(CodedInputStream* input, RoutingIndex* routingIndex, bool readOnlyRules) {
	...
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		case FlcNavRoutingIndex::kNameFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &routingIndex->name)));
			break;
		}
		case FlcNavRoutingIndex::kRulesFieldNumber: {
			int len;
		...
		case FlcNavRoutingIndex::kRootBoxesFieldNumber:
//		case FlcNavRoutingIndex::kBasemapBoxesFieldNumber:
			{
			if(readOnlyRules){
				input->Seek(routingIndex->filePointer + routingIndex->length);
				break;
			}
			bool basemap = false;//WireFormatLite::GetTagFieldNumber(tag) == FlcNavRoutingIndex::kBasemapBoxesFieldNumber;
		...
		case FlcNavRoutingIndex::kBlocksFieldNumber: {
			// Finish reading
			input->Seek(routingIndex->filePointer + routingIndex->length);
			break;
		}
...
bool readMapIndex(CodedInputStream* input, MapIndex* mapIndex, bool onlyInitEncodingRules) {
	...
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		case FlcNavMapIndex::kNameFieldNumber: {
			DO_((WireFormatLite::ReadString(input, &mapIndex->name)));
			break;
		}
		case FlcNavMapIndex::kRulesFieldNumber: {
		...
		case FlcNavMapIndex::kLevelsFieldNumber: {
			MapRoot mapLevel;
...
bool initMapStructure(CodedInputStream* input, BinaryMapFile* file) {
	...
	while ((tag = input->ReadTag()) != 0) {
		...
		case FlcNavStructure::kMapIndexFieldNumber: {
			MapIndex mapIndex;
		...
		case FlcNavStructure::kRoutingIndexFieldNumber: {
...
MapDataObject* readMapDataObject(CodedInputStream* input, MapTreeBounds* tree, SearchQuery* req,
	...
	unordered_map< std::string, unsigned int> stringIds;
	bool loop = true;
	while (loop) {
...
//		flcnav_log_print(LOG_INFO, "TODO Object is %llu  (%llu) ignored %s %s", (id + baseId) >> 1, baseId, types[0].first.c_str(), types[0].second.c_str());
//		return NULL;
//	}
...
bool searchMapTreeBounds(CodedInputStream* input, MapTreeBounds* current, MapTreeBounds* parent,
	...
	while ((tag = input->ReadTag()) != 0) {
		...
		case FlcNavMapIndex_MapDataBox::kShiftToMapDataFieldNumber : {
		...
		case FlcNavMapIndex_MapDataBox::kBoxesFieldNumber: {
			...
			if(results.size() > 0) {
				...
				for (std::vector<MapDataObject*>::iterator obj = results.begin(); obj != results.end(); obj++) {
					if ((*obj)->stringIds.size() > 0) {
						unordered_map<std::string, unsigned int >::iterator  val=(*obj)->stringIds.begin();
...
void convertRouteDataObjecToMapObjects(SearchQuery* q, std::vector<RouteDataObject*>& list, std::vector<MapDataObject*>& tempResult,
	...
	for (; rIterator != list.end(); rIterator++) {
		...
		if (add) {
			...
			obj->id = r->id;
			unordered_map<int, std::string >::iterator nameIterator = r->names.begin();
...
void readRouteDataAsMapObjects(SearchQuery* q, BinaryMapFile* file, std::vector<MapDataObject*>& tempResult,
	...
	for (; routeIndex != file->routingIndexes.end(); routeIndex++) {
		...
		for (std::vector<RouteSubregion>::iterator subreg = subs.begin(); subreg != subs.end(); subreg++) {
			if (subreg->right >= q->left && q->right >= subreg->left && subreg->bottom >= q->top
					&& q->bottom >= subreg->top) {
				LOGI( "Search route map %s", (*routeIndex)->name.c_str());
				contains = true;
			}
		}
		if (contains) {
		...
		for (std::vector<MapRoot>::iterator mapLevel = mapIndex->levels.begin(); mapLevel != mapIndex->levels.end();
			...
			if (mapLevel->minZoom <= q->zoom && mapLevel->maxZoom >= q->zoom) {
				if (mapLevel->right >= q->left && q->right >= mapLevel->left && mapLevel->bottom >= q->top
					...
					// flcnav_log_print(LOG_INFO, "Search map %s", mapIndex->name.c_str());
...
ResultPublisher* searchObjectsForRendering(SearchQuery* q, bool skipDuplicates, int renderRouteDataFile, std::string msgNothingFound) {
	...
	if (renderRouteDataFile >= 0) {
		...
		LOGI( "Route objects %d", tempResult.size());
	}
	...
	if (q->publisher->isCancelled()) {
		...
		LOGI("Search : tree - read( %d), accept( %d), objs - visit( %d), accept(%d), in result(%d) ",
...
bool readRouteDataObject(CodedInputStream* input, uint32_t left, uint32_t top, RouteDataObject* obj) {
	...
	while ((tag = input->ReadTag()) != 0) {
		...
		case RouteData::kPointJNIndFieldNumber:
			{
				uint32_t length;
				DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
				int oldLimit = input->PushLimit(length);
				
				while (input->BytesUntilLimit() > 0)//for(int i=0;i<length/2;i++)
				{
					uint32_t val;
					DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &val)));
					obj->bJNInd.push_back((unsigned short)val);
				}
				
				input->PopLimit(oldLimit);
			}
			break;
		case RouteData::kPointJNWayIdsFieldNumber:
			{
				uint32_t length;
				DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
				int oldLimit = input->PushLimit(length);
				/*
				int size=obj->bJNInd.size();
				if(size>0)
				{   // we've read Junction indexes in  <case RouteData::kPointJNIndFieldNumber:>
					//int index=
					for(int k=0;k<size;k++)
					{
						unsigned short v=obj->bJNInd[k];
						int nW=v&0xff;   // low byte is the number of waysIDs
						//  upper byte is the Node Index within this way
						for(int i=0;i<nW;i++)
						{
							uint64_t val;
							DO_((WireFormatLite::ReadPrimitive<uint64_t, WireFormatLite::TYPE_UINT64>(input, &val)));
							obj->bJNWayIds.push_back(val);
						}
					}
				}
				else
...
bool readRouteTreeData(CodedInputStream* input, RouteSubregion* s, std::vector<RouteDataObject*>& dataObjects, RoutingIndex* routingIndex) {
	...
	unordered_map<int64_t, std::vector<uint64_t> > restrictions;
	std::vector<std::string> stringTable;
	while ((tag = input->ReadTag()) != 0) {
		switch (WireFormatLite::GetTagFieldNumber(tag)) {
		// required uint32_t version = 1;
		case FlcNavRoutingIndex_RouteDataBlock::kDataObjectsFieldNumber: {
			uint32_t length;
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
			int oldLimit = input->PushLimit(length);
		...
		case FlcNavRoutingIndex_RouteDataBlock::kStringTableFieldNumber: {
			uint32_t length;
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
			int oldLimit = input->PushLimit(length);
			readStringTable(input, stringTable);
		...
		case FlcNavRoutingIndex_RouteDataBlock::kRestrictionsFieldNumber: {
			uint32_t length;
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
			int oldLimit = input->PushLimit(length);
			uint64_t from = 0;
		...
		case FlcNavRoutingIndex_RouteDataBlock::kIdTableFieldNumber: {
			uint32_t length;
			DO_((WireFormatLite::ReadPrimitive<uint32_t, WireFormatLite::TYPE_UINT32>(input, &length)));
			int oldLimit = input->PushLimit(length);
			int64_t routeId = 0;
	...
	unordered_map<int64_t, std::vector<uint64_t> >::iterator itRestrictions = restrictions.begin();
			...
			if ((*dobj)->namesIds.size() > 0) {
				...
				for(; itnames != (*dobj)->namesIds.end(); itnames++) {
					if((*itnames).second >= stringTable.size()) {
						LOGE( "ERROR VALUE string table %d", (*itnames).second );
					} else {
...
#endif
	if (fileDescriptor < 0) {
		LOGE( "Cache file could not be open to read : %s", inputName.c_str());
		return false;
	}
	FileInputStream input(fileDescriptor);
	CodedInputStream cis(&input);
	cis.SetTotalBytesLimit(INT_MAX, INT_MAX);
	FlcNavStoredIndex* c = new FlcNavStoredIndex();
	if(c->MergeFromCodedStream(&cis)){
		LOGI("Native Cache file initialized %s", inputName.c_str());
		cache = c;
...
BinaryMapFile* initBinaryMapFile(std::string inputName,bool flag) {
	GOOGLE_PROTOBUF_VERIFY_VERSION;
	std::map<std::string, BinaryMapFile*>::iterator iterator;
	if ((iterator = openFiles.find(inputName)) != openFiles.end()) {
		delete iterator->second;
	...
	if (fo != NULL) {
		...
		flcnav_log_print(SK_MESSAGE_DEBUG, "Native file initialized from cache %s", inputName.c_str());
	} else {
		FileInputStream input(fileDescriptor);
		input.SetCloseOnDelete(false);
		CodedInputStream cis(&input);
...
void clearOpenFiles(std::string inputName)
{
	std::map<std::string, BinaryMapFile*>::iterator iterator;
	if ((iterator = openFiles.find(inputName)) != openFiles.end()) {
		delete iterator->second;
...
//======================================================================================
//------------------------------------------------------------
#include "addressSearch_binaryRead.cpp"
//------------------------------------------------------------
