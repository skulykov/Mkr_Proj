...
int parseColor(string colorString) {
	if (colorString[0] == '#') {
		...
		if (colorString.size() == 7) {
			...
			//flcnav_log_print(LOG_ERROR, "Unknown color %s", colorString.c_str());
		}
		return (int) color;
	}
	//flcnav_log_print(LOG_ERROR, "Unknown color %s", colorString.c_str());
...
RenderingRule::RenderingRule(map<string, string>& attrs, RenderingRulesStorage* storage) {
	...
	for (; it != attrs.end(); it++) {
		...
		if (property == NULL) {
			//flcnav_log_print(LOG_ERROR, "Property %s was not found in registry", it->first.c_str());
			return ;
		}
...
RenderingRule* RenderingRulesStorage::getRule(int state, int itag, int ivalue) {
	unordered_map<int, RenderingRule*>::iterator it = (tagValueGlobalRules[state]).find(
...
void RenderingRulesStorage::registerGlobalRule(RenderingRule* rr, int state) {
	...
	if (tag == -1) {
		//flcnav_log_print(LOG_ERROR, "Attribute tag should be specified for root filter ");
	}
	...
	static void startElementHandler(void *data, const char *tag, const char **atts) {
		...
		if (len == 6 && "filter" == name) {
			...
			if (t->st.size() > 0 && t->st.top().isGroup()) {
				...
				//flcnav_log_print(LOG_ERROR, "Group filter without parent");
			}
			...
			if (t->dependsStorage != NULL) {
				...
				//flcnav_log_print(LOG_ERROR, "!Dependent rendering style was not resolved : %s", depends.c_str());
			}
			//renderingName = attrsMap["name"];
		} else {
			//flcnav_log_print(LOG_WARN, "Unknown tag : %s", name.c_str());
		}
...
void RenderingRulesStorage::printDebug(int state) {
	unordered_map<int, RenderingRule*>::iterator it = tagValueGlobalRules[state].begin();
...
void RenderingRulesStorage::parseRulesFromXmlInputStream(const char* filename, RenderingRulesStorageResolver* resolver) {
	...
	if (file == NULL) {
		//flcnav_log_print(LOG_ERROR, "File can not be open %s", filename);
	...
	if (depends != NULL) {
		...
		for (int i = 0; i < SIZE_STATES; i++) {
			...
			unordered_map<int, RenderingRule*>::iterator it = depends->tagValueGlobalRules[i].begin();
			for (; it != depends->tagValueGlobalRules[i].end(); it++) {
				unordered_map<int, RenderingRule*>::iterator o = tagValueGlobalRules[i].find(it->first);
...
RenderingRuleSearchRequest::RenderingRuleSearchRequest(RenderingRulesStorage* storage)  {
	...
	unordered_map<string, RenderingRuleProperty*>::iterator it = PROPS->properties.begin();
