#ifndef _FCNAV_RENDER_RULES_H
#define _FCNAV_RENDER_RULES_H
...
public:
	...
	unordered_map<string, RenderingRuleProperty*> properties;
	...
	inline RenderingRuleProperty* getProperty(const char* st) {
		unordered_map<std::string, RenderingRuleProperty*>::iterator i = properties.find(st);
		if (i == properties.end()) {
			return NULL;
		}
		return (*i).second;
...
//		R_TEXT_SHIELD = registerRuleInternal(RenderingRuleProperty::createOutputIntProperty("textShield"));
...
private:
	...
	unordered_map<std::string, int> dictionaryMap;
	std::vector<std::string> dictionary;
	const static int SHIFT_TAG_VAL = 16;
	// TODO make private
public:
	const static int SIZE_STATES = 7;
	unordered_map<int, RenderingRule*>* tagValueGlobalRules;
...
public:
	...
	RenderingRulesStorage(const void* storage, bool createDefProperties = true) : storageId(storage),
		...
		tagValueGlobalRules = new unordered_map<int, RenderingRule*>[SIZE_STATES];
	...
	inline int getDictionaryValue(std::string s) {
		unordered_map<std::string, int>::iterator it = dictionaryMap.find(s);
