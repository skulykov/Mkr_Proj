...
void ClsMqtt::send_message(char *topic, char *msg_payload)
{
	if(mclient)
		mclient->publish(NULL, topic, strlen(msg_payload), msg_payload, 1, false);
}
void ClsMqtt::send_login_message(char *topic, char *msg_payload)
{
	if(mclient)
		mclient->publish(NULL, topic, strlen(msg_payload), msg_payload, 1, true); // retain
}

void ClsMqtt::add_server_message(char *dest_id, char *msg, int flag)
{
...
int ClsMqtt::send2server(byte *msg_payload, int size)
{
	string my_did = Global::Get()->GetDriverID();
...
void ClsMqtt:: processComms()
{
	...
	if ((t - prev_hos_engine_time)>45)
	{
		prev_hos_engine_time = t;

		process_all_unsent_from_sqlite(0);
		LOGI("process_all_unsent_from_sqlite ...\n");
...
void *ClsMqtt::doIt()
{
	...
	mosqpp::lib_init();
	string my_client ="mqtt-"+ Global::Get()->GetDriverID();
...
void  ClsMqtt::doInitMqtt(int par1, char *payload)
{
	if (initialized)
	{
	...
	*/
	//createRenderingThread
	{
		...
		if (pthread_create(&thread, NULL, ClsMqtt::thread_helper_function, this) != 0)
		{
