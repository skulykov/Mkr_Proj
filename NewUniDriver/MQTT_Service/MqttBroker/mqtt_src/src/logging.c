...
int _mkr_log_vprintf(struct mosquitto *mosq, int priority, const char *fmt, va_list va)
{
	char msg[1024];
	vsprintf(msg, fmt, va);
	printf("%s \r\n", msg);
...
int _mosquitto_log_printf(struct mosquitto *mosq, int priority, const char *fmt, ...)
{
	...
	va_start(va, fmt);
	rc = _mkr_log_vprintf(mosq, priority, fmt, va);//_mosquitto_log_vprintf(mosq, priority, fmt, va);
	va_end(va);
...
void mosquitto_log_printf(int level, const char *fmt, ...)
{
	...
	va_start(va, fmt);
	_mkr_log_vprintf(NULL, level, fmt, va);//_mosquitto_log_vprintf(NULL, level, fmt, va);
	va_end(va);
