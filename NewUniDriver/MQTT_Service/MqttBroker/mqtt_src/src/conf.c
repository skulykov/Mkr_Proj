...
int mqtt3_config_parse_args(struct mqtt3_config *config, int argc, char *argv[])
{
	...
	//svk read conf file
#ifdef _DEBUG
	config->config_file = "C:\\My\\projects\\NewUniDriver\\MQTT_Service\\MqttBroker\\mqtt_broker.conf";
#else
	config->config_file = "C:\\inetpub\\mqtt_broker\\mqtt_broker.conf"; 
#endif
	if(!config->config_file){
		_mosquitto_log_printf(NULL, MOSQ_LOG_ERR, "Error: Out of memory.");
		return MOSQ_ERR_NOMEM;
